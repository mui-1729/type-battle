...
- `apps/cloudflare-worker/src/worker.ts` は room ごとの Durable Object へ振り分ける構成で、room 単位の状態管理を想定している。
...
- D1 Free: database `10` 個、database size `500 MB`、account storage `5 GB`、queries per Worker invocation `50`、database ごとの同時接続は `6`。
...
- 1 room authoritative update = 1 DO invocation 相当
...
逆に、room snapshot を typing update ごとに D1 へ書くのは避けるべき。
...
DO 単体の soft limit は `1,000 requests / sec` なので、1 room の typing 負荷は通常ここには届かない。

この repo の 2 人対戦で、1 room が 1 人あたり毎秒 5 〜 10 回程度更新するとしても、room 単位ではまだかなり余裕がある。

ただし、room 数が増えると Workers Free の daily request 先に当たりやすい。

### D1 Free

guest session と match result を中心に保存するなら、D1 の `50 queries / Worker invocation` は十分。

D1 が危険になるのは次のケース。

- typing update ごとに D1 に書く
- room state snapshot を毎回永続化する
- イベントログを D1 に積み続ける

そのため、D1 は「低頻度の永続化」に限定するのが前提。
...
DO の per-object soft limit は、2 人対戦中心の private beta なら優先ボトルネックになりにくい。

## throttling / coalescing 方針

### 必須

- D1 書き込みは room create / join / match result などの低頻度イベントに限定する
- room snapshot を keydown ごとに永続化しない
