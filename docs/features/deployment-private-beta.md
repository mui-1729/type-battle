...
この repo には realtime server の `Dockerfile`、`fly.toml`、`tests/smoke-test.ts` があり、デプロイ前後の health / socket 確認はできる。
...
realtime については Fly.io 向けの config と deploy workflow を repo で管理する。
